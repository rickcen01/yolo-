...
def main():
    ...
    for file in files:
        ...
        try:
            ...
            if result.masks:
                for i, mask in enumerate(result.masks.data):
                    ...
                    if cls_id in SOURCE_CLASS_IDS:
                        ...
                        # Must be at least 5% of the image (removes small noise like in Pic_gas_1)
                        if area_ratio < 0.05:
                            continue
                            
                        # FILTER 2: Centrality
                        # Must be reasonably central (e.g., within middle 50% of image)
                        # dist_ratio 0.0 means center, 0.5 means corner.
                        # Let's say max dist_ratio 0.3
                        if dist_ratio > 0.35:
                            continue
            ...
            if candidates:
                ...
                # We prioritize centrality over confidence because the cylinder is always the main subject
                candidates.sort(key=lambda x: x['dist_ratio'])
                
                # Pick the most central one
                selected_candidate = candidates[0]
                
                # Exception: If the most central is 'chair' or 'sink' but there is a 'bottle' 
                # very close by (e.g., slightly less central but correct class), prefer bottle?
                # For now, let's stick to simple centrality. 
                # In Pic_gas_183, it's a "toilet" but central. "Bottle" might not be detected at all.
                # So we must accept whatever class is central.
            ...
            if selected_candidate:
                ...
                if len(polygon_norm) >= 3:
                    ...
                    print(f"  Saved match: {cls_name} (Conf {conf:.2f}, Dist {selected_candidate['dist_ratio']:.2f})")
                    success_count += 1
                else:
                    print("  Invalid polygon (points < 3)")
            else:
                print("  No valid objects found after filtering.")
                # Clear file if exists? User complained about wrong detections, so yes, safer to clear.
                # But actually, if we fail, maybe better to leave empty than wrong.
                with open(txt_path, 'w') as f:
                    pass # Empty file
    ...
    print(f"Done. Processed {count} images, successfully segmented {success_count}.")
